...
import { Table, Tag, Button, Space, Input, Select, Tooltip, Popconfirm, Badge, message } from 'antd'
...
export default function Tokens() {
  ...
  const { data, isLoading, refetch } = useTokens(filter)
  const revoke = useRevokeToken()

  const handleRevoke = (tokenId: string, prefix: string) => {
    revoke.mutate(tokenId, {
      onSuccess: () => messageApi.success(`Token ${prefix} revoked`),
      onError: () => messageApi.error('Failed to revoke token'),
    })
  }

  // Client-side search across token prefix, username, client_id, scope
  ...
  return (
    <div style={{ padding: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 13, color: 'var(--color-text-muted)' }}>
              {data?.total ?? 0} token{(data?.total ?? 0) !== 1 ? 's' : ''} found
            </p>
          </div>
        </div>
        <Button
      ...
      <div style={{
        ...
        {/* Active toggle */}
        <Select
          value={filter.active === false ? 'all' : 'active'}
          onChange={val => setFilter(f => ({ ...f, active: val === 'all' ? false : true }))}
          style={{ width: 160 }}
          options={[
        ...
        <Input
          ...
          onChange={e => setFilter(f => ({ ...f, client_id: e.target.value || undefined }))}
          allowClear
        ...
        {(filter.client_id || filter.user_id || search) && (
          <Button
            size="small"
            onClick={() => { setFilter({ active: filter.active }); setSearch('') }}
            style={{ borderRadius: 6 }}
          >
            Clear filters
          </Button>
